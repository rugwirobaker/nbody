...
**Amended by:** [RFC-002](RFC-002.md) — the merge trigger is contact-based (§2.6, Step 10)
**Language:** Zig · **Renderer:** hand-written WebGL2 over a `wasm32-freestanding` build (amended 2026-08-19; the original entry read *raylib via raylib-zig*)
...
There is no special case for a stationary particle: $v_{x_0} = 0$ is just a value. Acceleration never acts on position directly; it acts on velocity, and velocity acts on position, so particles curve toward masses instead of beelining.
...
**Cost:** each of $n$ particles sums over $n-1$ sources → $n(n-1) \approx n^2$ pair evaluations per tick. **O(n²) is the computational shadow of the superposition principle.** Micro-optimization (including SIMD) shrinks the constant; only a different physics *approximation* (e.g. Barnes–Hut, Part 4) changes the exponent.
...
- The cancellation holds **in the discrete algorithm**, not just the continuous physics — *provided* Phase A used one frozen snapshot (Step 8). That is what makes momentum a usable acceptance test (§2.5).
...
**Rule to specify:** when two particles come closer than a threshold, replace them with one. (The threshold is defined by [RFC-002](RFC-002.md) §1.2; every formula below is unchanged by it.) Every field of the merged particle is **forced** by conservation laws, not chosen:
...
All three are the same mass-weighted-average shape. Un-weighted averages fail the momentum test on the first merge, which is what that test is for.
...
Physically the lost energy becomes internal heat ("the splat"). The simulation records it with one optional per-particle scalar `heat`, incremented by $\Delta KE$ at each merge. This captures the kinetic loss exactly. It leaves out the merged pair's mutual potential, which departs the system along with the pair, so **kinetic + gravitational potential + total heat** steps up slightly at each merge instead of holding constant — around 0.3 % of total energy per merge at the default config. §2.5's energy test is written around that. `heat` also gives a free visual channel (heat → brightness, with slow decay = radiative cooling; violent mergers flare, gentle ones barely glow).
...
- The Phase-A inner loop body is ~14 arithmetic operations including one `sqrt` and one `div` (the two expensive ones), executed $n^2$ times per tick. This sentence is Part 2's closing line and Part 3's opening line: SIMD's offer is the same fourteen operations processing $L$ pairs each.
...
2. The reported metric is **Phase A wall time per tick, in nanoseconds**, timed around Phase A with a monotonic clock — never FPS; rendering must not contaminate the physics number. (`std.time.Timer` was removed in Zig 0.16; the harness uses `std.Io.Timestamp.now(io, .awake)`.)
...
| (d) Energy | two-body orbit, ~10⁴ ticks, `heat_decay = 1.0` | orbit radius does not systematically grow; with merging: each merge's $KE$ step $=$ its recorded $\Delta KE$ exactly, and $KE + PE + \sum heat$ flat on merge-free ticks | Step 5 ordering; Step 10 |
...
> **Amended by [RFC-002](RFC-002.md).** The trigger is now a contact test,
> $d^2 < (r_i + r_j)^2$ with $r(m) = k\sqrt{m}$, so a body's drawn size is its
> merge cross-section. The fixed threshold and its softening rationale below are
> the original rule, kept for the record; RFC-002 §3 says why the rationale did
> not survive. The three forks — placement, chains, removal — are unchanged and
> remain in force.
...
**Alignment (should):** allocate SoA arrays 64-byte aligned (a cache line, ≥ any vector width in play). Modern cores tolerate unaligned vector loads at near-full speed, so this is hygiene rather than correctness. It is free at allocation time and removes a variable from benchmark interpretation.
...
**Performance expectations:** theoretical ceiling $L\times$; reality lower, because `sqrt`/`div` have reduced vector throughput on most cores. **4–7× at L = 8 is success.** Deliverable: ns/tick (Phase A) vs $n$, log-log, both builds, same seed sweep.

**Measured (2026-08-19):** 2.90× at $L = 4$ (aarch64/NEON) and 5.80× at $L = 8$ (x86-64/AVX2), which is 0.725 of lane width on both. The cache term did not bind on either machine: ns/pair stayed flat to $n = 16384$, where a row streams ~390 KB of AoS particles, because the access is sequential and the per-pair `sqrt`+`div` gives the prefetcher ample time. The AVX2 shortfall against $L\times$ traces to the `sqrt`/`div` term instead — per-doubling gain measured 2.00×, 1.92×, 1.80×.
